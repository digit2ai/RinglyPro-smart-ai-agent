...
def parse_natural_datetime(text: str) -> Optional[datetime]:
    ...
    # Handle "X minutes" (without "in") - THIS WAS MISSING!
    if re.search(r'^(\d+) minutes?$', text):
        match = re.search(r'^(\d+) minutes?$', text)
        minutes = int(match.group(1))
        result = now + timedelta(minutes=minutes)
        print(f"[DEBUG DATETIME] '{minutes} minutes' -> {result}")
        return result
    ...
    # Handle "X hours" (without "in")
    if re.search(r'^(\d+) hours?$', text):
        match = re.search(r'^(\d+) hours?$', text)
        hours = int(match.group(1))
        result = now + timedelta(hours=hours)
        print(f"[DEBUG DATETIME] '{hours} hours' -> {result}")
        return result
